...
def SVGImageListView(ListView):
    paginate_by = 5
    model = SVGImage

def thumbnails(request):
    images = SVGImage.objects.all()
    return render(request, 'polls/thumbnails.html', {'images': images})

def sorted_by_tags(request):
    images = SVGImage.objects.order_by('tags__name')
    return render(request, 'polls/sorted_by_tags.html', {'images': images})
...
def home(request):
    ...
    if tag_id:
        image_list = SVGImage.objects.filter(tags__id=tag_id)
    else:
        image_list = SVGImage.objects.all()

    if sort_order == 'desc':
        image_list = image_list.order_by('-pub_date')
    else:
        image_list = image_list.order_by('pub_date')
